...
const item = Array.isArray(res.item)
            ? res.item.map((elem) => ({ _id: elem._id, details: elem.details }))
            : res.item
                ? [{ _id: res.item._id, details: res.item.details }]
                : [];
        setItem(item);
            })
            .catch((error) => {
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Item</label>
                    {item.map((elem) => (
                        <Link key={elem._id} to={`/items/${elem._id}`}>
                        <div>
                  {" "}
                            <p className="text-xl text-primary">{elem.details}</p>
                            </div>
                        </Link>
                    ))}</div>
